...
class TransactionLogFilterTest {
    ...
    @Test
    void usesIncomingTransactionIdWhenHeaderPresent() throws Exception {
        String incomingTxId = "incoming-tx-12345";
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader(TransactionLogFilter.TRANSACTION_ID_HEADER, incomingTxId);
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = mock(FilterChain.class);
        ...
        assertEquals(incomingTxId, capturedTxId.get());
        assertEquals(incomingTxId, response.getHeader(TransactionLogFilter.TRANSACTION_ID_HEADER));
    }
